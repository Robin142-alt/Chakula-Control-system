...
4. Open `Issue stock` and point out:
   ...
   - preset quantities
   - backfill mode
5. Open `Backfill CSV` and show one pasted paper row importing into the local queue.
6. Switch to the Cook role and open `Log leftovers`.
7. Show how little typing is required.
8. Switch to the Principal role and show:
   - today's cost
   - cost per student
   - top 3 high alerts only
9. End on Reports and explain the seven-day anomaly story.
